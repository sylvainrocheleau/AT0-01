...
class MetaSpider(scrapy.Spider):
    ...
    # match_filter = {"type": "bookie_and_comp", "params":["1XBet", "BundesligaAlemana"]}
    # match_filter = {"type": "comp", "params":["LaLigaEspanola"]}
    match_filter = {"type": "match_url", "params":["https://sports.williamhill.es/betting/es-es/f%C3%83%C2%BAtbol/OB_EV35037573/alemania-%C3%A2%E2%80%9A%E2%80%B9-portugal"]}
    matches_details_and_urls = Helpers().matches_details_and_urls(
        filter=match_filter_enabled,
        filter_data=match_filter
        # scrapping_tool=self.scraping_tool
    )
    matches_details_and_urls = {
        key: [match for match in value if match['scraping_tool'] in ['requests', 'playwright', 'scrape_ops', 'zyte_proxy_mode']] for
    ...
    def start_requests(self):
        ...
        for key, value in self.matches_details_and_urls.items():
            counter = 0
            for data in value:
                try:
