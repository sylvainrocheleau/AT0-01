...
class TwoStepsSpider(scrapy.Spider):
    name = "comp_spider_01"
    custom_time_out = 3600 # in seconds
    debug = False
    match_url = str
    comp_url = str
    proxy_ip = str
    ...
    # TODO change load_matches_urls to faster method
    ...
    def start_requests(self):
        ...
        try:
            if os.environ["USER"] in LOCAL_USERS:
                ...
                # competitions = [x for x in bookie_config(bookie=["Bwin"]) if x["competition_id"] == "SegundaDivisionEspanola"]
        except:
            competitions = bookie_config(bookie=["all_bookies"])

        competitions = [x for x in competitions if x["scraping_tool"] in ["playwright", "scrape_ops", "requests", "zyte_proxy_mode"]]
    ...
    async def match_requests(self,response):
        ...
        # TODO test if page close can be close right after ('found playwright')
    ...
    def closed(self, reason):
        print("spider closed finally", reason)
