...
class TwoStepsSpider(scrapy.Spider):
    ...
    async def match_requests(self,response):
        ...
        for xpath_result in xpath_results:
            try:
                ...
                home_team = xpath_result.xpath("//span[contains(@class, 'text-sm truncate pr-1')]/text()").extract()[0]
                home_team = home_team.strip()
                away_team = xpath_result.xpath("//span[contains(@class, 'text-sm truncate pr-1')]/text()").extract()[1]
                ...
                if response.meta.get("competition") == "NBA":
                    match_infos.append(
                        {"url": "https://www.bet777.es" + url, "home_team": away_team, "away_team": home_team,
                         "date": date})
                else:
                    match_infos.append(
        ...
        for match_info in match_infos:
            ...
            # if match_info["url"] == "https://www.bet777.es/futbol/england-premier-league/manchester-city-vs-brentford-25321029/":
    ...
    async def parse_match(self, response):
        ...
        try:
            ...
            item["user_agent_hash"] = response.meta.get("user_agent")
            if len(item["Bets"]) > 1:
                yield item
    ...
    async def errback(self, failure):
        ...
        item["user_agent_hash"] = failure.request.meta.get("user_agent")
