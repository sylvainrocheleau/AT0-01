...
from ..utilities import Connect, build_ids, build_new_match_info, normalize_team_names
...
class TwoStepsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        context_infos = get_context_infos(bookie_name=self.real_name)
        self.context_infos = [x for x in context_infos if x["proxy_ip"] not in []]
        for data in bookie_config(self.real_name):
    ...
    async def match_requests(self,response):
        ...
        web_url_prefix = "https://1xbet.es/line/"
        for match in json_responses["Value"]:
            try:
                url = url_prefix + str(match["CI"])
                web_url = web_url_prefix + str(
                    match["SE"] + "/" + str(match["LI"]) + "-" +
                    match["LE"].replace(".", "") + "/" +
                    str(match["CI"]) + "-" + match["O1E"] + "-" + match["O2E"]).replace(" ", "-")
                home_team = match["O1E"]
                away_team = match["O2E"]
                date = datetime.datetime.fromtimestamp(match["S"])
                match_infos.append(build_new_match_info(
                    url=url, web_url=web_url, home_team=home_team,away_team=away_team, date=date,
                    competition_id=response.meta.get("competition").replace(" ", ""),
                    bookie_id=self.real_name, sport=response.meta.get("sport")
                )
                )
            except IndexError:
                continue
        ...
        if self.mode == "comp_only":
            match_infos.append(build_new_match_info(
                url="http://allnew.com", web_url="http://allnew.com", home_team="Home", away_team='IpswichTown',
                date=datetime.datetime.utcnow(),
                competition_id="PremierLeagueInglesa",
                bookie_id="1XBet", sport="Football"
            )
            )
            match_infos.append(build_new_match_info(
                url="http://close.com", web_url="http://close.com", home_team="FC Leicester City",
                away_team='CrystalPalace', date=datetime.datetime.utcnow(),
                competition_id="PremierLeagueInglesa",
                bookie_id="1XBet", sport="Football"
            )
            )
            match_infos.append(build_new_match_info(
                url="http://existing_home_not_away.com", web_url="http://existing_home_not_away.com",
                home_team="Crystal Palace", away_team='Ipswich Town', date=datetime.datetime.utcnow(),
                competition_id="PremierLeagueInglesa",
                bookie_id="1XBet", sport="Football"
            )
            )
            match_infos.append(build_new_match_info(
                url="http://all_existing.com", web_url="http://all_existing.com", home_team="Crystal Palace",
                away_team='Manchester United', date=datetime.datetime.utcnow(),
                competition_id="PremierLeagueInglesa",
                bookie_id="1XBet", sport="Football"
            )
            )
            match_infos = normalize_team_names(match_infos=match_infos)
